...
if __name__ == '__main__':
    ...
    # num_dims = 3 specifies x,y,z coordinates of
    # each point in the point cloud
    sim_data = Variable(torch.rand(32, 3, 2500))
    trans = STN3d()
    out = trans(sim_data)
    print('stn', out.size())  # (batch_size, 3, 3)
    # for an affine transformation matrix to transform
    # the pointclouds.
    sim_data = sim_data.transpose(2, 1)
    print('sim_data', sim_data.size())  # (batch_size, num_points, 3)
    x = torch.bmm(sim_data, out)  # batch matrix-matrix product of matrices
    print('x', x.size())  # (batch_size, num_points, 3) preserves
    # the dim after affine transformation.
